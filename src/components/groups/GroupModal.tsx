  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (activeGroup) {
        ...
        if (onSuccess) onSuccess(activeGroup.id);
      } else {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-900 dark:text-white">
                {activeGroup ? t('groupModals.editGroupTitle') : t('groupModals.createGroupTitle')}
              </h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                {activeGroup ? 'Update group title, schedule, or assigned teacher' : 'Set up a new cohort and schedule'}
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder={t('groupModals.groupNamePlaceholder')}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-800 dark:text-slate-200 mb-1">
              {t('groupModals.scheduleDaysLabel')}
            </label>
          ...
          <div className="pt-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-end gap-2">
            ...
            >
              {loading ? t('common.saving') : activeGroup ? t('groupModals.updateButton') : t('groupModals.createButton')}
            </button>
          </div>
        </form>
      </div>
